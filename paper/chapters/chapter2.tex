...
A mutex (mutual exclusion lock) ensures that at most one thread or process executes a critical section at any given time~\cite{herlihy2012}. Implementations range from pure spin locks---which avoid context switches but waste CPU cycles during contention---to fully blocking locks that yield to the scheduler at the cost of 1--10 microsecond wake-up latency. A hybrid approach spins briefly before falling back to blocking, exploiting the common case where locks are held only briefly. This spin-then-block strategy, described in Section~\ref{sec:futex}, forms the basis of the futex-based lock used in \texttt{rs\_ipc}.

\subsection{Condition Variables}
\label{subsec:condvars}

While mutexes protect shared state, they do not provide a mechanism for threads to efficiently wait for state changes. Condition variables fill this gap by allowing a thread to atomically release a lock and suspend execution until another thread signals that the condition may have changed~\cite{herlihy2012}. The key operations are \texttt{wait} (atomic release-and-sleep), \texttt{notify\_one} (wake a single waiter), and \texttt{notify\_all} (wake all waiters). The atomicity of the release-and-suspend in \texttt{wait} prevents the ``lost wakeup'' problem, where a notification arrives between releasing the lock and entering the wait state.
...
Rust's ownership system prevents use-after-free, double-free, dangling pointers, and data races at compile time through single-ownership semantics and exclusive-or-shared borrowing rules~\cite{rustbook}. Two marker traits extend these guarantees to concurrent programming: \texttt{Send} (a type can be safely transferred between threads) and \texttt{Sync} (a type can be safely shared between threads via references). For shared memory structures, manually implementing \texttt{Sync}---with an accompanying safety justification---is the mechanism by which Rust verifies that concurrent access is correctly synchronized.

Operations that the compiler cannot verify---raw pointer dereferencing, foreign function calls, manual \texttt{Sync} implementations---require the \texttt{unsafe} keyword~\cite{rustonomicon}. The convention is to encapsulate these operations within safe abstractions whose public API upholds the required invariants. In a shared memory library, typical unsafe operations include constructing references from \texttt{mmap}-returned pointers, implementing \texttt{Sync} for structures containing \texttt{UnsafeCell}, and invoking operating system interfaces through FFI. High-level abstractions such as iterators and generics compile to code equivalent to hand-written C, ensuring that these safety guarantees introduce no runtime overhead~\cite{rustbook}.
